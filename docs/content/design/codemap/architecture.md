...
<dl class="cm-kv">
  ...
  <dt>Events</dt><dd>`internal/agent/events.go`. The agent decides what happened; the caller decides how it looks. `cliEvents` renders line output, `tcellEvents` drives the full-screen UI, both from the same callbacks. It is a structured sink called from the single run goroutine and callable through teardown; a run panic returns a `PanicError` and reports the stack through `Panicked`, never on the returned error.</dd>
  <dt>Prompter</dt><dd>`internal/toolkit/prompter.go`. The only path allowed to reach an operator, whether through a terminal or another channel. A line implementation (`survey_prompter.go`) and a full-screen one (`internal/tui/prompter.go`) satisfy it; `CanPrompt` reports whether an operator is reachable, and deny-by-default lives at the caller, never in the prompter.</dd>

...
var _ = Describe("runstate", func() {
	...
	Describe("FileStore", func() {
		...
		newMeta := func(id string) MetaRecord {
			return MetaRecord{Version: Version, RunID: id, Prompt: "hello", Fingerprint: Fingerprint{Model: "claude-opus-4-8"}}
		}
		...
		It("creates, appends, and folds back a run", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(j.Append(2, Record{Protocol: AssistantProtocol, Assistant: assistantWithTools(0, "tu_1")})).To(Succeed())
			Expect(j.Append(3, Record{Protocol: ToolResultProtocol, ToolResult: toolResult("tu_1")})).To(Succeed())
		...
		It("refuses to create a run that already exists", func() {
			...
			Expect(err).To(MatchError(ErrExists))
		})
		...
		It("treats a duplicate seq as an idempotent no-op and rejects gaps", func() {
			...
			Expect(j.Append(5, Record{Protocol: ToolResultProtocol, ToolResult: toolResult("tu_1")})).To(MatchError(ErrSeqGap))
		})
		...
		It("rejects unsafe run ids (path traversal)", func() {
			...
			Expect(err).To(MatchError(ErrInvalidID))
		...
		It("does not leak a sensitive prompt into the fingerprint on disk", func() {
			id := newID()
			meta := newMeta(id)
			meta.Fingerprint.SystemHash = HashHex([]byte("TOP-SECRET-INSTRUCTIONS"))

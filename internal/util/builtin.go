...
// to a *Tool, which runs a command of the introspected fisk application. Built-in
...
type BuiltinTool struct {
	...
	handler func(ctx context.Context, input json.RawMessage, prompter Prompter) (string, error)
...
func (b *BuiltinTool) ToolParam() anthropic.ToolUnionParam {
	return anthropic.ToolUnionParam{OfTool: &anthropic.ToolParam{
		Type:         anthropic.ToolTypeCustom,
		Name:         b.name,
		Description:  anthropic.String(b.description),
		DeferLoading: anthropic.Bool(false),
		InputSchema:  anthropicInputSchema(b.schema),
	}}
}
...
// ExecuteBuiltinUse runs a built-in tool for a model tool_use block and returns
// the matching tool_result content block, mirroring ExecuteToolUse for command
// tools: a handler error becomes an error result; its JSON output becomes a normal
// result.
func ExecuteBuiltinUse(ctx context.Context, b *BuiltinTool, use anthropic.ToolUseBlock, prompter Prompter) anthropic.ContentBlockParamUnion {
	out, err := b.handler(ctx, use.Input, prompter)
	if err != nil {
...
// StdinIsTerminal reports whether the agent's stdin is an interactive terminal,
// the condition the human-in-the-loop tools need to reach a person.
func StdinIsTerminal() bool { return stdinIsTerminal() }
...
func askHumanConfirm(ctx context.Context, input json.RawMessage, prompter Prompter) (string, error) {
	var args struct {
	...
	if !stdinIsTerminal() {
		return outcomeJSON(askHumanConfirmName, confirmOutcome{Reason: noTerminalReason})
	}
...
func askHumanSelect(ctx context.Context, input json.RawMessage, prompter Prompter) (string, error) {
	var args struct {
		Question string   `json:"question"`
	...
	if !stdinIsTerminal() {
		return outcomeJSON(askHumanSelectName, selectOutcome{Reason: noTerminalReason})
	}
	if err := ctx.Err(); err != nil {
...
func askHumanInput(ctx context.Context, input json.RawMessage, prompter Prompter) (string, error) {
	var args struct {
		Question string `json:"question"`
	...
	if !stdinIsTerminal() {
		return outcomeJSON(askHumanInputName, inputOutcome{Reason: noTerminalReason})
	}
	if err := ctx.Err(); err != nil {
...
// noTerminalReason is the reason returned when a human-in-the-loop tool is called
// with no interactive terminal attached to reach an operator.
...
func sanitizePrompt(s string) string {
	return sanitizeForTerminal(s, maxPromptRunes)
}

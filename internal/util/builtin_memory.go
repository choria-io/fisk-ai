...
func MemoryIndexBlock(entries []memory.Item) string {
	...
	for _, e := range entries {
		desc := sanitizeForTerminal(e.Description, maxIndexDescriptionRunes)
...
func memoryKeyTrace(name string) func(json.RawMessage) string {
	return func(input json.RawMessage) string {
		...
		key := sanitizeForTerminal(args.Key, maxIndexDescriptionRunes)
		if key == "" {
			return name
		}
...
func memoryListHandler(store memory.Store) builtinHandler {
	return func(ctx context.Context, _ json.RawMessage, _ Prompter) (string, error) {
		if store == nil {
...
type builtinHandler = func(ctx context.Context, input json.RawMessage, prompter Prompter) (string, error)
